...
```bash
# Build it directly (bypasses sandbox network)
...
## Kernel Module Build Fails Silently

If `linux-*-modules.drv` fails but building it directly succeeds:

```bash
# Build the modules derivation directly
